...
export default function LotteriesPage() {
  ...
  const handleImageUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setScanStatus("Scanning text and digits from image...");
      const ret = await worker.recognize(file);
      const text = ret.data.text;
      await worker.terminate();

      setScanStatus("Extracting numbers...");

      // 6-digit numbers (1st Prize)
      const sixDigits = text.match(/\b\d{6}\b/g);
      if (sixDigits && sixDigits.length > 0) {
        setNewFirstPrize(sixDigits[0]);
      }

      // 3-digit numbers
      const threeDigits = text.match(/\b\d{3}\b/g);
      if (threeDigits && threeDigits.length > 0) {
        if (threeDigits.length >= 2) {
          setNewFront3(threeDigits[0]);
          setNewBack3(threeDigits[1]);
        } else {
          setNewBack3(threeDigits[0]);
        }
      }

      // 2-digit numbers
      const twoDigits = text.match(/\b\d{2}\b/g);
      if (twoDigits && twoDigits.length > 0) {
        setNewTwoDigits(twoDigits[twoDigits.length - 1]);
      }
      ...
      if (dateMatch) {
        try {
          ...
          if (!isNaN(parsed.getTime())) {
            setNewDrawDate(parsed.toISOString().slice(0, 10));
          }
        } catch (_) {}
      }

      setScanStatus("✨ Scan completed! Numbers auto-filled below.");
    } catch (err: any) {
          ...
          <div className="glass-panel" style={modalContentStyle}>
            ...
            >
              ...
              <div style={{ fontSize: "0.85rem", fontWeight: 700, color: "var(--accent-cyan)" }}>
                Upload Lottery Sheet / Photo (OCR Auto-Fill)
              </div>
              <div style={{ fontSize: "0.75rem", color: "var(--text-muted)" }}>
                Click or drop an image to automatically extract draw numbers
              </div>
              {scanStatus && (
                <div style={{ fontSize: "0.8rem", color: "#60a5fa", fontWeight: 600, marginTop: "0.25rem" }}>
