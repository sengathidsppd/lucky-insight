...
def map_job_to_response(job: AnalysisJob, db: Session, user: Optional[User] = None) -> AnalysisJobResponse:
    ...
    if job.result:
        ...
        if isinstance(res_dict, dict):
            ...
            if is_superadmin or is_operator_admin:
                # Admins (Super Admin & Operator Admin): 1x 6D (Rank #1) for both Thai and Lao, 3x 2D (Rank #1, #2, #3)
                if "best_analyzed_6d" in res_dict and isinstance(res_dict["best_analyzed_6d"], list) and len(res_dict["best_analyzed_6d"]) > 0:
                    res_dict["best_analyzed_6d"] = res_dict["best_analyzed_6d"][:1]
                elif "superadmin_picks_6d" in res_dict and isinstance(res_dict["superadmin_picks_6d"], list) and len(res_dict["superadmin_picks_6d"]) >= 1:
                    res_dict["best_analyzed_6d"] = res_dict["superadmin_picks_6d"][:1]

                # Remove 4D for Admins
                res_dict.pop("generated_4d_recommendations", None)

                # 2D: Top 3 sets (Rank #1, #2, #3)
                ...
                # Regular User: No 6D, No 4D, 3x 2D (Rank #1, #2, #3)
                res_dict.pop("generated_4d_recommendations", None)
