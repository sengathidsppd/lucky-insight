"""Test auto check logic for tickets."""
...
def main():
    ...
    # Get pending tickets
    cur.execute("""
        SELECT ut.id, ut.draw_date, ut.lottery_type, ut.number_code, ut.status
        FROM user_tickets ut
        WHERE ut.status = 'PENDING';
    """)
    pending = cur.fetchall()
    print(f"Found {len(pending)} PENDING tickets:")
    for t in pending:
        t_id, draw_date, l_type, code, status = t
        print(f"Checking Ticket: {t_id} | Date: {draw_date} | Type: {l_type} | Code: {code}")
        ...
        if result:
            ...
            print(f"  Found Draw Result: 1st={first_prize}, last4={last4}, back3={back3}, last2={last2}")
            new_status, match_info = check_ticket(code, first_prize, last4, back3, last2, front3)
            print(f"  ==> OUTCOME: {new_status} ({match_info})")
        else:
